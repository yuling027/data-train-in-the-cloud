...
# ean.drop("fare_amount", axis=1)
#     y = data_clean[["fare_amount"]]
    
     
#     X_processed = preprocess_features(X)
    
#     data_with_time_stamp = pd.DataFrame(np.concatenate((
...
#     # data_train = data.iloc[:train_length, :].sample(frac=1).to_numpy()
#     # data_val = data.iloc[train_length:, :].sample(frac=1).to_numpy()
...
def train(
    ...
    if data_processed.shape[0] < 10:
        print(":x: Not enough processed data retrieved to train on")
        return None
    # $CHA_END
    # Create (X_train_processed, y_train, X_val_processed, y_val)
    # $CHA_BEGIN
    ...
    # Save model weight on the hard drive (and optionally on GCS too!)
    save_model(model=model)
    print(":white_check_mark: train() done \n")
    return val_mae
